<?php
    class User{
        ...
        public function save_user(){
            require_once 'Db.php';
            $db = Db::getInstance();
            $sql = "update `users` set `u_username`='{$this->u_username}',`u_password`='{$this->u_password}',`u_sex`='{$this->u_sex}',`u_mail`='{$this->u_mail}',`u_phone`='{$this->u_phone}',`u_flag`='{$this->u_flag}',`u_role`='{$this->u_role}',`u_code`='{$this->u_code}',`u_time`='{$this->u_time}',`u_tips`='{$this->u_tips}' where `u_id`='{$this->u_id}';";
            $res = $db->write($sql);
            if ($res!=false){
                return true;
            }else{
         ...
         * @param mixed $u_flag
         */
        public function setUFlag($u_flag)
        {
            $this->u_flag = $u_flag;
        }
